  ...
  return (
    <div className="max-w-md mx-auto px-5 pt-4 pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h1 className="text-xl font-bold">Fertilizer advisor</h1>
      </div>

      <Card>
        <form onSubmit={submit} className="space-y-4">
          <div>
            <Label>Which crop?</Label>
            <Combobox
              options={CROP_NAMES}
              value={form.cropName}
      ...
      {advice && (
        <div className="mt-5 space-y-3">
          <Card>
            <CardTitle>Nutrient status for {advice.crop}</CardTitle>
                  ...
                  >
                    ...
                    <div>
                      <p className={cn("font-semibold", color)}>
                        {d.status === "deficient"
                          ? `Add ${d.deficitKgPerHa} kg/ha`
                          : d.status === "excess"
                            ? `Excess (${Math.abs(d.deficitKgPerHa)} kg/ha over)`
                            : "Adequate"}
                      </p>
                      <p className="text-xs text-brand-mute">
                        Current {d.currentKgPerHa} · Target {Math.round(d.targetKgPerHa)}
                      </p>
                    </div>
                    <div
                      className={cn(
          ...
          {advice.products.length > 0 && (
            <Card>
              <CardTitle>Buy this fertilizer (per acre)</CardTitle>
              <ul className="mt-3 space-y-2.5">
                {advice.products.map((p) => (
                  <li key={p.name} className="border-l-4 border-brand-primary pl-3">
                    <p className="font-semibold">
                      {p.bagsPerAcre} bag{p.bagsPerAcre !== 1 ? "s" : ""} of {p.name}
                    </p>
                    <p className="text-xs text-brand-mute">
                      ({p.bagSizeKg} kg each) · {p.note}
                    </p>
                  </li>
                ))}
              </ul>
          ...
          <Card className="bg-brand-primary/5">
            <p className="text-sm">
              🌱 <b>Organic tip:</b> {advice.organicTip}
            </p>
          </Card>
        </div>
      )}
